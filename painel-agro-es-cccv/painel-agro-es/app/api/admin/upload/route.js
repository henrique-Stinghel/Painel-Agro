...
export async function POST(request) {
  ...
  if (!process.env.BLOB_READ_WRITE_TOKEN) return Response.json({ error: 'Armazenamento de imagens não configurado.' }, { status: 503 });
  ...
  const blob = await put(`anuncios/${crypto.randomUUID()}-${safeName}`, file, { access: 'public', addRandomSuffix: false });
